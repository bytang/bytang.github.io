...
(function (namespace) {
  ...
  namespace.Clock = function Clock() {
    this.last = NaN;
    this.baseTime = 0;
    this.speed = 1;
  };

  clock = namespace.Clock.prototype;
  ...
  namespace.Clock.prototype.importSettings = function importSettings(settingsArray) {
    this.last = settingsArray[0];
    this.baseTime = settingsArray[1];
    this.speed = settingsArray[2];
  };

  namespace.Clock.prototype.exportSettings = function exportSettings() {
    return [this.last, this.baseTime, this.speed];
  };

  namespace.Clock.prototype.reset = function reset() {
  ...
  namespace.Clock.prototype.start = function start() {
    if (isNaN(this.last)) {
  ...
  namespace.Clock.prototype.stop = function stop() {
    this.baseTime = this.time;
    this.last = NaN;
